...
# Step 1: Collect basic patient info
...
# Step 2: Presenting complaint selection
...
# Step 3: Symptom details based on complaint (branching)
    ...
    if complaint == "Toothache":
        ...
        st.session_state.answers["pain_severity"] = st.selectbox(
            "Pain severity", ["Mild", "Moderate", "Severe"], index=["Mild", "Moderate", "Severe"].index(st.session_state.answers.get("pain_severity", "Mild"))
        )
        st.session_state.answers["pain_trigger"] = st.radio(
            "Is the pain triggered by:", ["Cold", "Heat", "Sweet foods", "Chewing", "None"], index=["Cold", "Heat", "Sweet foods", "Chewing", "None"].index(st.session_state.answers.get("pain_trigger", "None"))
        )

    elif complaint == "Bleeding gums":
        st.session_state.answers["bleeding_frequency"] = st.selectbox(
            "How often do your gums bleed?", ["Rarely", "Sometimes", "Often", "Always"], index=["Rarely", "Sometimes", "Often", "Always"].index(st.session_state.answers.get("bleeding_frequency", "Rarely"))
        )
        ...
        st.session_state.answers["pain_with_swelling"] = st.radio(
            "Is the swelling painful?", ["Yes", "No"], index=["Yes", "No"].index(st.session_state.answers.get("pain_with_swelling", "No"))
        )
